...
def s_box_output(s_box_number, input_bit):
    row = (input_bit[0] << 1) + input_bit[5]
    column = (input_bit[1] << 3) + (input_bit[2] << 2) + (input_bit[3] << 1) + input_bit[4]
    output = S_box[s_box_number][16 * row + column]
    return bytearray([int(bit) for bit in format(output, '04b')])
...
round_func_work = round_func(ip_work, key_bitarray1)
