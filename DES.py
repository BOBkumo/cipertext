#def feistal_cipher(text, num_rounds, key):
#    left, right = text[:len(text)//2], text[len(text)//2:]
#
#    for round in range(num_rounds)
#        update_right = xor_strings(left)

IPT = [58, 50, 42, 34, 26, 18, 10, 2,
       60, 52, 44, 36, 28, 20, 12, 4,
       62, 54, 46, 38, 30, 22, 14, 6,
       64, 56, 48, 40, 32, 24, 16, 8,
       57, 49, 41, 33, 25, 17, 9, 1,
       59, 51, 43, 35, 27, 19, 11, 3,
       61, 53, 45, 37, 29, 21, 13, 5,
       63, 55, 47, 39, 31, 23, 15, 7]

FPT = [40, 8, 48, 16, 56, 24, 64, 32,
       39, 7, 47, 15, 55, 23, 63, 31,
       38, 6, 46, 14, 54, 22, 62, 30,
       37, 5, 45, 13, 53, 21, 61, 29,
       36, 4, 44, 12, 52, 20, 60, 28,
       35, 3, 43, 11, 51, 19, 59, 27,
       34, 2, 42, 10, 50, 18, 58, 26,
       33, 1, 41, 9, 49, 17, 57, 25]

PBDT = [57, 49, 41, 33, 25, 17, 9,
        1, 58, 50, 42, 34, 26, 18,
        10, 2, 59, 51, 43, 35, 27,
        19, 11, 3, 60, 52, 44, 36,
        63, 55, 47, 39, 31, 23, 15,
        7, 62, 54, 46, 38, 30, 22,
        14, 6, 61, 53, 45, 37, 29,
        21, 13, 5, 28, 20, 12, 4]

CPBT = [14, 17, 11, 24, 1, 5, 3, 28,
        15, 6, 21, 10, 23, 19, 12, 4,
        26, 8, 16, 7, 27, 20, 13, 2,
        41, 52, 31, 37, 47, 55, 30, 40,
        51, 45, 33, 48, 44, 49, 39, 56,
        34, 53, 46, 42, 50, 36, 29, 32]

EPT = [32, 1, 2, 3, 4, 5,
       4, 5, 6, 7, 8, 9,
       8, 9, 10, 11, 12, 13,
       12, 13, 14, 15, 16, 17,
       16, 17, 18, 19, 20, 21,
       20, 21, 22, 23, 24, 25,
       24, 25, 26, 27, 28, 29,
       28, 29, 30, 31, 32, 1]

S_box = [
    # S1  
    [
        [14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7],
        [0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8],
        [4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0],
        [15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13]
    ],
    # S2
    [
        [15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10],
        [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5],
        [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15],
        [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9]
    ],
    # S3
    [
        [10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8],
        [13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1],
        [13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7],
        [1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12]
    ],
    #S4
    [
        [7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15],
        [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9],
        [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4],
        [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14]
    ],
    #S5
    [
        [2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9],
        [14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6],
        [4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14],
        [11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3]
    ],
    #S6
    [
        [12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11],
        [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8],
        [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6],
        [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13]
    ],
    #S7
    [
        [4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1],
        [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6],
        [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2],
        [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12]
    ],
    #S8
    [
        [13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7],
        [1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2],
        [7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8],
        [2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11]
    ]
]

SPT = [16, 7, 20, 21, 29, 12, 28, 17,
       1, 15, 23, 26, 5, 18, 31, 10,
       2, 8, 24, 14, 32, 27, 3, 9,
       19, 13, 30, 6, 22, 11, 4, 25]

def s_box_output(s_box_number, input_bit):
    output = []

    row = (s_box_number[0] << 1) + s_box_number[5]
    column = (s_box_number[1] << 3) + (s_box_number[2] << 2) + (s_box_number[3] << 1) + s_box_number[4]

    output = input_bit[row][column]
    binary = bin(output)[2:].zfill(4)

    return bytearray([int(bit) for bit in binary])

def change_to_bit(plain: str):
    bit2 = ''
    count = 0
    for d in plain:
        bit = bin(ord(d))[2:]
        for c in bit2:
            if c == 1:
                count += 1
            else:
                continue
        if (count % 2) == 0:
            bit1 = bit + str(0)
        
        else:
            bit1 = bit + str(1)
        bit2 += bit1
    
    return bit2

'''
11행 입력받은 문자열을 이진수로 변환
12행 for문은 각 이진수의 문자를 보고 1일 경우 카운트업
17행 카운트값이 짝수일 경우, 7자리 이진수 값에 0을 추가
     카운트값이 홀수일 경우, 7자리 이진수 값에 1을 추가
     패리티비트를 추가한 코드.(짝수 패리티비트)
'''

def bit_to_bitarray(plain: str):
    bit = change_to_bit(plain)
    bitarray = bytearray([int(i) for i in bit])
    return bitarray

def bitarray_to_bit(bitarray: bytearray):
    return "".join([str(i) for i in bitarray])

def bitarray_to_plain(bitarray: bytearray):
    bit = bitarray_to_bit(bitarray)
    pass

def permutation(text: bytearray, table: list[int], length: int):
    permutated_text = bytearray(length)
    
    for num in range(len(table)):
        point = table[num]-1     #IP_table은 1부터 시작하지만 text의 인덱스는 0부터 시작하기때문에 -1 수행
        permutated_text[num] = text[point]

    return permutated_text


def key_made(key: bytearray):

    round_key = []

    #순열을 시켜 패리티비트 제거
    pc_1_table = permutation(key, PBDT, 56)
    left = pc_1_table[:28]
    right = pc_1_table[28:]

    #shift 작업
    shift_1 = [0,1,8,15]                           #1씩 shift 시킴.(1,2,9,16라운드에)
    
    for i in range(16):
        if i in shift_1:
            left = left[1:] + left[0:1]
            right = right[1:] + right[0:1]

        else:
            left = left[2:] + left[0:2]
            right = right[2:] + right[0:2]
        round_key.append(permutation(left+right, CPBT, 48))
    return round_key

def round_func(result, round_key):
    left_plain = result[:32]
    right_plain = result[32:]
    
    for round in range(16):
        expansion = permutation(right_plain, EPT, 48)

        for bit in range(48):
            expansion[bit] ^= round_key[round][bit]

        s_box_work = bytearray(32)

        for block_index in range(8):
            s_box_work[4*block_index:4*block_index+4] = s_box_output(expansion[6*block_index:6*block_index+6], S_box[block_index])

        strighted = permutation(s_box_work, SPT, 32)
        '''
        bytearray 자료형의 strighted 와 left_plain을 xor 연산하기 위해 bytes의 자료형으로 전환 후 xor 연산 진행.
        '''
        right_plain = bytearray(a ^ b for a,b in zip(left_plain, strighted))
    '''
    round함수 종료 후 right_plain 32bit 와 left_plain 32bit를 더해 최종 결과값을 도출.
    그 후 FPT 에 넣어 순열을 진행시킨다.
    '''
    round_func_result = right_plain + left_plain
    return round_func_result


key = input("key 값을 입력해라. : ")
key_bitarray = change_to_bit(key)
key_bitarray1 = bit_to_bitarray(key_bitarray)
round_key = key_made(key_bitarray1)
round_key_to_bit = bitarray_to_bit(round_key)
#print(round_key_to_bit)
#print(type(round_key_to_bit))
#print(is_2d_array(round_key))

text = input("평문을 입력해라. : ")
bitarray = change_to_bit(text)
bytearray1 = bit_to_bitarray(bitarray)
'''
print(f"bytearray 변경문 출력: {bit_to_bitarray(bitarray)}")
print("길이: ", len(bytearray1))
'''
ip_work = permutation(bytearray1, IPT, 64)
round_func_work = round_func(ip_work, round_key)
des_result = permutation(round_func_work, FPT, 64)
result = bitarray_to_bit(ip_work)
print("실행결과: ", result)
